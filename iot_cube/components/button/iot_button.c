...
static button_dev_t *button_create_com(uint8_t active_level, uint8_t (*hal_get_key_state)(void *usr_data), void *usr_data)
{
    ...
    if (false == g_is_timer_running) {
        ...
        int ret = esp_timer_start_periodic(g_button_timer_handle, TICKS_INTERVAL * (1000 * (1000 / CONFIG_FREERTOS_HZ)));
        ESP_LOGI(TAG, "esp_timer_start_periodic:%s", esp_err_to_name(ret));
